...
1.  **Add as Git Submodule:**
    ...
    ```bash
    git submodule add https://github.com/23jinny/fortenum.git third_party/fortenum
    ```
    *(You can adjust the path `third_party/fortenum` if you prefer a different location like `external/` or `vendor/`.)*

2.  **Integrate with CMake:**

    In your main `CMakeLists.txt`:
    ```cmake
    # Add the fortenum subdirectory (adjust path if you changed it above)
    add_subdirectory(third_party/fortenum)

    # Link your target against fortenum
    target_link_libraries(your_application_target PRIVATE fortenum)
    ```
    *   `fortenum` is an `INTERFACE` library. Linking with `PRIVATE` is generally recommended and means `your_application_target` can use `fortenum`. If other parts of your project also need `fortenum`, they would link to it directly. For more details on `PUBLIC` or `INTERFACE` linking (e.g., if `your_application_target` is a library that exposes `fortenum` types in its own API), please see the "Advanced CMake Integration" section below.
...
3.  **Define Your Enums in Fortran:**
    ...
    module mod_something
      use fortenum, only: EnumBase_t  ! Import the base type
      implicit none
      private
...
### Solution: Using fortenum Enums

With fortenum, you can define a type-safe enumeration for `InteractionType_e` (something more specific like `ParticleInteractionType_e` should be more appropriate, but this is a mouthful for this example):
...
By using enums, you make your Fortran code more robust, readable, and maintainable. fortenum provides the tools to easily create and use such type-safe enumerations in your projects.

---

## Detailed Explanations & Advanced Topics

### Why was fortenum Created?
...
fortenum addresses these by providing a base `abstract type` (`EnumBase_t`) from which specific enumeration types can be derived. This ensures that values from different enumerations cannot be inadvertently mixed or compared, as the compiler will enforce type checking.
...
-   **CMake for Building**: The project uses CMake for its build system. CMake is a powerful, cross-platform build system generator that is widely adopted and well-suited for Fortran projects. It simplifies the process of integrating `fortenum` into other CMake-based projects as a submodule.
...
**Understanding CMake Keywords for `fortenum`:**

The `fortenum` library is defined as an `INTERFACE` library in its own `CMakeLists.txt`:
```cmake
# In fortenum/CMakeLists.txt:
add_library(fortenum INTERFACE)
target_sources(fortenum INTERFACE "${CMAKE_CURRENT_SOURCE_DIR}/src/fortenum.f90")
target_include_directories(fortenum INTERFACE "${CMAKE_CURRENT_SOURCE_DIR}/src")
```
-   `add_library(fortenum INTERFACE)`: This declares that `fortenum` doesn't compile into a separate library file. Instead, its properties (like source files and include directories) are "interfaced" to consuming targets.
-   `target_sources(fortenum INTERFACE ...)`: This specifies that `fortenum.f90` is a source file that consuming targets need to compile.
-   `target_include_directories(fortenum INTERFACE ...)`: This specifies that the `src` directory (containing `fortenum.f90`) should be added to the include path of consuming targets. The `INTERFACE` keyword here means this include directory is part of `fortenum`'s public interface and is *always* propagated to any target that links against `fortenum`, regardless of whether that link is `PRIVATE`, `PUBLIC`, or `INTERFACE`.

When you link your target to `fortenum` using `target_link_libraries`:
```cmake
target_link_libraries(your_application_target <PRIVATE|PUBLIC|INTERFACE> fortenum)
```
The keywords `PRIVATE`, `PUBLIC`, and `INTERFACE` control how the *usage requirements* of `fortenum` (like its include directories and source files defined with `INTERFACE` in its own `CMakeLists.txt`) are propagated *further* if `your_application_target` is itself a library.

*   **`PRIVATE`**: If `your_application_target` links to `fortenum` with `PRIVATE`, it means `your_application_target` uses `fortenum` for its internal implementation. The properties of `fortenum` are applied to `your_application_target`. However, if another target (e.g., `final_executable`) links to `your_application_target` (which is a library), `fortenum`'s properties are *not* propagated to `final_executable`. If `final_executable` also needs `fortenum`, it must link to `fortenum` directly.

*   **`PUBLIC`**: If `your_application_target` (a library) links to `fortenum` with `PUBLIC`, it means `fortenum` is part of `your_application_target`'s public API. The properties of `fortenum` are applied to `your_application_target`, AND they are also propagated to any target that links to `your_application_target`. This allows those downstream targets to also `use fortenum` without linking to `fortenum` explicitly.

*   **`INTERFACE`**: If `your_application_target` is *itself* an `INTERFACE` library and links to `fortenum` with `INTERFACE`, it means `fortenum`'s properties are added to `your_application_target`'s own interface properties, to be propagated to whatever links to `your_application_target`.

In summary:
-   The `INTERFACE` keyword in `target_sources` and `target_include_directories` within `fortenum/CMakeLists.txt` defines *what* `fortenum` provides to direct consumers.
...
**Multiple Targets Depending on `fortenum`:**

If multiple targets (e.g., several libraries and executables) within your larger project need to use `fortenum`:
1.  Call `add_subdirectory(third_party/fortenum)` once, typically in a high-level `CMakeLists.txt` file if possible. If `add_subdirectory` for `fortenum` is called multiple times with the same submodule path, CMake usually handles this correctly by processing the `fortenum` project only once, ensuring the `fortenum` target is defined uniquely.
2.  Each target that directly uses `fortenum` (i.e., has `use fortenum` in its Fortran source) must explicitly link to `fortenum` via `target_link_libraries(... fortenum)`.

Because `fortenum` is an `INTERFACE` library, there are no conflicting compiled library files. CMake ensures that `fortenum.f90` is compiled as needed by each consuming target and that the Fortran module (`fortenum.mod`) is correctly found.
